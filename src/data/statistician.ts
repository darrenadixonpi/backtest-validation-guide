...
export const HYPOTHESIS_TESTS: HypothesisTest[] = [
  ...
  },
  {
    ...
    nullHypothesis:
      'Not a sharp $H_0$ test — estimates sampling distribution of $\\widehat{SR}$ (or mean) under temporal dependence.',
  ...
  },
  {
    ...
    nullHypothesis:
      '$H_0$: $E[d_{m,t}] \\leq 0$ for all strategies $m$ vs benchmark (no strategy beats benchmark).',
    statistic: '$T_M = \\max_m \\sqrt{T}\\,\\bar{d}_m$',
    bootstrapOrNull: 'Stationary bootstrap on centered loss differentials under $H_0$.',
    rejectionMeans:
      'At least one strategy beats benchmark after adjusting for selecting the max — FWER controlled.',
...
export const ASSUMPTIONS: AssumptionItem[] = [
  {
    ...
    howToCheck: 'Audit split code; verify $\\max(T_{\\mathrm{train}}) < \\min(T_{\\mathrm{test}})$ per fold.',
  },
  {
    id: 'purge-h',
    category: 'Leakage',
    assumption:
      'Purge distance $P_k \\geq \\max(H, L_f)$ for label horizon $H$ and feature lookback $L_f$.',
  ...
  },
  {
    ...
    category: 'Leakage',
    assumption:
      'Embargo $E > 0$ when returns/features are autocorrelated even if $H = 0$.',
    whyItMatters: 'Serial correlation transmits information across fold borders.',
    howToCheck: 'Inspect ACF of strategy returns; increase $E$ if $\\rho_1$ significant.',
  },
  {
  ...
  },
  {
    ...
    howToCheck: 'Stress $\\lambda_c \\in \\{1, 2, 3\\}$ on costs; forward test reconciliation.',
  },
  {
    id: 'panel-cluster',
